...
if __name__ == '__main__':
    ...
    def find_model_files(directory):
        """Recursively search for model files in directory and its subdirectories"""
        for root, dirs, files in os.walk(directory):
            print(f"Searching in directory: {root}")
            print(f"Files found: {files}")
            for filename in files:
                if re.match(r"model_world_size_(\d+)_rank_0\.pt", filename):
                    return root, int(re.match(r"model_world_size_(\d+)_rank_0\.pt", filename).group(1))
        return None, None
    
    print(f"Searching for model files in: {local_dir}")
    model_dir, world_size = find_model_files(local_dir)
    
    if not model_dir or not world_size:
        raise ValueError(
            f"No model file with the proper format 'model_world_size_{{N}}_rank_0.pt' found in {local_dir} or its subdirectories."
        )
    
    print(f"Found model files in: {model_dir} with world_size: {world_size}")
    local_dir = model_dir  # Update local_dir to the directory containing the model files
